...
tf.app.flags.DEFINE_string('test_data_path',
                           'D:/blister/Blister_test_video/blister_train_for&back/FastROR_recog_dataset/JPEGImages/', '')
# tf.app.flags.DEFINE_string('test_data_path', 'D:/FOTS/FOTS_TF-dev/datasets/tf3_dataset/test/JPEGImages/', '')
tf.app.flags.DEFINE_string('gpu_list', '0', '')
tf.app.flags.DEFINE_string('checkpoint_path', './blister_checkpoints/loc_recog_roi16_0208/', '')
tf.app.flags.DEFINE_string('output_dir', 'D:/blister/Blister_test_video/blister_train_for&back/FastROR_recog_dataset/Annotations/', '')
...
def detect_train(score_map, geo_map, score_map_thresh=0.8, box_thresh=0.1, nms_thres=0.2):
    '''
    restore text boxes from score map and geo map
    :param score_map:
    :param geo_map:
...
def main(argv=None):
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            cls = listdir(FLAGS.test_data_path)

            for c in cls:

                img_src = "{}{}".format(FLAGS.test_data_path, c)
                img_back_src = "{}/back/".format(img_src)
                img_for_src = "{}/for/".format(img_src)

                # back img read
                im_fn_list = get_images(img_back_src)
                timing = []
                for im_fn in im_fn_list:
                    ...
                    # sort the boxes to true dirtection
                    if boxes.shape[0] >= 2:
                    ...
                    if boxes is not None and boxes.shape[0] != 0:
                        res_file_dst = "{}{}/back/".format(FLAGS.output_dir, c)
                        if not os.path.exists(os.path.dirname(res_file_dst)):
                            os.makedirs(os.path.dirname(res_file_dst))
                        res_file_path = os.path.join(res_file_dst,
                    ...
                    if not FLAGS.no_write_images:
                        img_path = os.path.join(res_file_dst, os.path.basename(im_fn))
                        # cv2.imwrite(img_path, im[:, :, ::-1])
                        if im_txt is not None:
                            cv2.imwrite(img_path, im_txt)

                # for img read
                im_fn_list = get_images(img_for_src)
                timing = []
                for im_fn in im_fn_list:
                    im = cv2.imread(im_fn)[:, :, ::-1]
                    start_time = time.time()
                    ...
                    timer['detect'] = time.time() - start
                    if boxes is not None and boxes.shape[0] != 0:
                        res_file_dst = "{}{}/for/".format(FLAGS.output_dir, c)
                        if not os.path.exists(os.path.dirname(res_file_dst)):
                            os.makedirs(os.path.dirname(res_file_dst))
